...
ArgParse supports four kinds of options: boolean, string, integer, and floating-point.
...
Options can be preceded, followed, or interspaced with positional arguments. ArgParse assembles all positional arguments together into a single, undifferentiated array of strings. Convenience functions are supplied for parsing these strings as integers or doubles.

ArgParse supports the standard `--` switch for turning off option-parsing. All arguments following a `--` will be treated as positional arguments, even if they begin with a single or double dash.



### Commands

ArgParse supports git-style command interfaces with arbitrarily-nested commands. Commands have builtin support for an automatic `--help` flag and an automatic `help <cmd>` command, i.e. the commands
